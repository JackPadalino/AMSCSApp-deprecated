...
""" # test for the approving/denying helpful requests, notifying question and answer authors, and incrementing user solutions count
class MakeSuperuserHelpfulRequest(TestCase):
    def setUp(self):
        user1 = User.objects.create_user(username='NewUser1',email='newuser@gmail.com',password='newest_user!',first_name='Snerd',last_name='Snerdyson')
        user2 = User.objects.create_user(username='NewUser2',email='newuser@gmail.com',password='newest_user!',first_name='Snerd',last_name='Snerdyson')
        user3 = User.objects.create_superuser(username='SuperUser1',email='newuser@gmail.com',password='newest_user!',first_name='Snerd',last_name='Snerdyson')
        ...
        forum_topic = ForumTopic.objects.create(title='New Topic',forum=forum)
        question1 = Question.objects.create(author=user1,forum=forum,forum_topic=forum_topic,title='Question 1',content='This is my first question!',project_link='')
        answer1 = Answer.objects.create(author=user2,forum=forum,forum_topic=forum_topic,question=question1,content='Here is my answer!',requested=True)
        #AnswerNotification.objects.create(answer=answer1,notified_user=user3)

    def test(self):
        user1 = User.objects.get(username='NewUser1')
        user2 = User.objects.get(username='NewUser2')
        user3 = User.objects.get(username='SuperUser1')
        #question1=Question.objects.get(author=user1,title='Question 1')
        #answer1=Answer.objects.get(author=user2,question=question1,content='Here is my answer!',requested=True)
        user1Notifications=AnswerNotification.objects.filter(notified_user=user1)
        user2Notifications=AnswerNotification.objects.filter(notified_user=user2)
        user3Notifications=AnswerNotification.objects.filter(notified_user=user3)
        self.assertEqual(user1.profile.solutions,0)
        self.assertEqual(user2.profile.solutions,0)
        self.assertEqual(len(user1Notifications),1)
        self.assertEqual(len(user2Notifications),0)
        self.assertEqual(len(user3Notifications),0)
        

class MarkAnswerSolutionTest(TestCase):
    def setUp(self):
        user1 = User.objects.create_user(username='NewUser1',email='newuser@gmail.com',password='newest_user!',first_name='Snerd',last_name='Snerdyson')
        user2 = User.objects.create_user(username='NewUser2',email='newuser@gmail.com',password='newest_user!',first_name='Snerd',last_name='Snerdyson')
        user3 = User.objects.create_superuser(username='SuperUser1',email='newuser@gmail.com',password='newest_user!',first_name='Snerd',last_name='Snerdyson')
        #Profile.objects.create(user=user1,solutions=1)
        #Profile.objects.create(user=user2,solutions=1)
        school_year = SchoolYear.objects.create(year=2022)
        classroom = Classroom.objects.create(title='Class 1',school_year=school_year,join_code='abc123')
        forum = Forum.objects.create(title='Forum 1',classroom=classroom)
        forum_topic = ForumTopic.objects.create(title='New Topic',forum=forum)
        question1 = Question.objects.create(author=user1,forum=forum,forum_topic=forum_topic,title='Question 1',content='This is my first question!',project_link='')
        answer1 = Answer.objects.create(author=user2,forum=forum,forum_topic=forum_topic,question=question1,content='Here is my answer!',requested=True,reviewed=True,solution=True)
    ...
    def test(self):
        ...
        user3 = User.objects.get(username='SuperUser1')
        question1=Question.objects.get(author=user1,title='Question 1')
        answer1=Answer.objects.get(author=user2,question=question1,content='Here is my answer!')
        self.assertEqual(answer1.solution,True)
        self.assertEqual(user1.profile.solutions,1)
        self.assertEqual(user2.profile.solutions,1) """